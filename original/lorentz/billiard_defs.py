...
def KE(part):
    ...
    ang_vel = np.asarray([[spin_vector_from_matrix(M) for M in particles] for particles in part.spin_hist])
...
def run_trial(wall, part):
    ...
    for step in range(part.max_steps):
        ...
        part.dt = min(part.dt_pp.min(), part.dt_wp.min())
        if np.isinf(part.dt):
...
def smoother(part, min_frames=None):
    ...
    if (min_frames is None):
        ...
        nominal_frame_length = dts[~short_step].min() / min_frames
...
def draw_hist(wall, part, duration=10):
    ...
    def draw(steps=1):        
        ...
        cell_range = [2 * s * np.arange(c.min(), c.max()+1) for (s,c) in zip(part.cell_size, cell)]
